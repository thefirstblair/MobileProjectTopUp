...
public class Product2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        productName.setText(getString(R.string.product2Name));
        ...
        buyBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (checkMoney(25) == true)
                {
                    createProduct();
                    createUserProduct();
                    updateData(25);
                    Toast.makeText(Product2Activity.this, "ซื้อสินค้าเรียบร้อย", Toast.LENGTH_SHORT).show();
                }
                else{
                    Toast.makeText(Product2Activity.this, "ไม่สามารถซื้อสินค้าได้ โปรดตรวจสอบยอดเงินอีกครั้ง", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    public void createProduct(){
        ...
        product.put("name", getString(R.string.product2Name));
        product.put("price", 25);
        product.put("detail", getString(R.string.product2Description));
    ...
    public void createUserProduct(){
        ...
        userProduct.put("productName", getString(R.string.product2Name));
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful() && !task.getResult().isEmpty()){
                    ...
                    fStore.collection("users").document(userId).update(map).addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void unused) {
                            Log.i("Update Money", "onSuccess: update money");
                        }
                    }).addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
                            Log.i("Update Money", "onFailure: update money");
                        }
                    });
                }else {
                    Toast.makeText(Product2Activity.this, "Failed!", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
